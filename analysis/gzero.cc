...
void gofr(int natoms,double x[],double cell,HISTOGRAM &hist)
{
  ...
  for (i=0;i<natoms;i++){
    for(k=0;k<DIM;k++) x[i*DIM+k] -= anint(x[i*DIM+k]*rcell)*cell;
  }
  for (i=0;i<natoms;i++){
    for(k=0;k<DIM;k++) dx[k]=x[i*DIM+k]-pos[k];
...
void save(int natoms,int nsave,double avvol,HISTOGRAM &hist)
{
  ...
  double dx = hist.dx(); 
  double fr=avvol/(4.*M_PI*dx*(double)(natoms*nsave)); // N not N^2
...
int main(int argc,char **argv)  /* pass in command line arguments */
{
  ...
  save(natoms,nsave,avvol/nsave,hist);
