...
static double *sig6,*eps4,rcut2;

/*! 
  freeLJScr frees the scratch space used by LJ routines
*/
void freeLJScr(void){
...
/*! 
  checkLJScr check that there is enough scratch space and if
  not then adds some to the scratch space
*/
static void checkLJSrc(int nintr){
  if(nintr>nxdis){
    freeLJScr();
...
/*! 
  frees the static memory in LJ energy routines, 
  both scratch and interactions types
*/
void freeLJS(void){
  freeLJScr();
  if(ntypes != 0) free(eps4);
}

/*! 
   LJsetup sets up LJ interactions
...
*/
void LJsetup(int ntyps,double eps[],double sig[],double rcut,double xbox)
{
  // eps = 0; no LJ
...
*/
double pot_lj(int nintr,double xi[],int idx[])
{
  int i,ii,k;
  double pot=0.;
...
*/
double force_lj(int nintr,double xi[],int idx[])
{
  int i,ii,id,k;
